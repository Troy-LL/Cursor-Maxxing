...
3. **Plan, then seat.** Show the plan (create / edit / omit / dump fate). Then seat facts using [owners.md](owners.md) and [map.md](map.md). Non-empty owners: patch in place; merge the new fact; keep the rest. A full-file replace of a living owner fails this step. Create only when the path is empty or missing. A dump heading named Definitions, Glossary, or Names folds into the matching owner — do not mint `docs/glossary.md`. Done when each kept job has one owner and the map bullets only files that exist.

...
Pack orientation: `/cursormax`. Marketplace: `.cursor-plugin/plugin.json`. Workflow skills: `.cursor/skills/`. Knobs: `.cursor/commands/`. Priors: `.cursor/rules/`. Product docs: `/sdd`. Behavior change: `/sdd-eng` (troysdd plugin — do not copy those SKILL.md files here). Do not add MCP that duplicates Grep, Glob, or Read. Do not add a router skill.
